...
# from instagram_searchV1 import Instagram
...
class ProfileSearch:
    ...
    def __init__(self):
        ...
        self.collection = db.instgram_search
        self.dict = {}
        self.obj = Instagram()

    def db_check(self, query):

        r = self.obj.search_api(query)
        print(r)
        t = 0
        for i in r:
  ...
  # ---------------------fetching total number of query pages from database----------------------------------------
    def db_fetch(self, query):
        self.collection.create_index([("name", "text"), ("userid", "text")])
